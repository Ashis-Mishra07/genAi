...
export default function CustomerDashboardPage() {
  ...
  const loadDashboardData = async () => {
    try {
      ...
      if (productsResponse.ok) {
        ...
        if (productsData.success) {
          ...
          const transformedProducts = productsData.products.map((product: any) => ({
            ...
            name: product.name,
            price: product.price,
            originalPrice: undefined, // Can be calculated if needed
            imageUrl: product.imageUrl || product.posterUrl || "",
            category: product.category || "Handcrafts",
            ...
            discount: undefined // Can be calculated if original price exists
          }));
      ...
      const statsResponse = await fetch('/api/orders');
      if (statsResponse.ok) {
      ...
      // If API calls fail, fall back to localStorage data
      if (!dashboardStats) {
        const cartItems = JSON.parse(localStorage.getItem('cart') || '[]');
        const wishlistItems = JSON.parse(localStorage.getItem('wishlist') || '[]');
      ...
      // Set empty arrays for products and orders on error
      setFeaturedProducts([]);
      setRecentOrders([]);
    } finally {
      setIsLoading(false);
