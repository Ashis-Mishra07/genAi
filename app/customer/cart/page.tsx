        ...
        <div className="max-w-7xl mx-auto">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-muted-foreground text-lg">{cartItems.length} handcrafted items ready for checkout</p>
            </div>
            ...
            <div className="flex items-center space-x-4">
              <div className="flex items-center space-x-3 bg-muted/50 backdrop-blur-sm rounded-xl px-4 py-3 border border-border">
                ...
                <div>
                  <p className="text-sm font-semibold text-card-foreground">Total: ₹{totals.total.toLocaleString()}</p>
        ...
        {isLoading ? (
          <div className="text-center py-20">
            <div className="flex flex-col items-center space-y-4">
              ...
              />
              <p className="text-muted-foreground text-lg">Preparing your selected items...</p>
            </div>
          </div>
        ) : error ? (
          <div className="text-center py-20">
            ...
            <h2 className="text-3xl font-bold text-foreground mb-4">Cart Loading Error</h2>
          ...
          <div className="text-center py-20">
            ...
            <p className="text-muted-foreground text-lg mb-8 max-w-md mx-auto">Discover amazing handcrafted products from talented artisans</p>
            <button
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
            ...
            <div className="lg:col-span-2 space-y-6">
              {cartItems.map((item) => (
                <div key={item.id} className="group bg-card backdrop-blur-sm rounded-2xl p-6 border border-border shadow-xl hover:border-primary/50 transition-all duration-300">
                  <div className="flex items-center space-x-4">
                    {/* Product Image */}
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-start justify-between">
                        <div>
                          <h3 className="font-bold text-card-foreground text-xl group-hover:text-primary transition-colors">{item.name}</h3>
                          <p className="text-muted-foreground text-sm font-medium">by {item.artisanName}</p>
                        ...
                        <div className="text-right">
                          <div className="flex items-center space-x-2">
                            <span className="text-2xl font-bold bg-gradient-to-r from-primary via-primary/80 to-primary bg-clip-text text-transparent">₹{item.price.toLocaleString()}</span>
                          </div>
                          {!item.inStock && (
                      ...
                      <div className="flex items-center justify-between mt-4">
                        <div className="flex items-center space-x-4">
                          <div className="flex items-center bg-muted/50 rounded-xl border border-border">
                            ...
                            <span className="px-4 py-3 text-card-foreground font-bold min-w-[50px] text-center bg-muted">{item.quantity}</span>
                            <button
                              onClick={() => updateQuantity(item.id, 1)}
                          ...
                          <span className="text-card-foreground font-bold text-lg">₹{(item.price * item.quantity).toLocaleString()}</span>
                        </div>
              ...
              >
                <div className="flex items-center justify-center space-x-3">
                  ...
                  <span className="font-semibold">✨ Discover More Amazing Products</span>
                </div>
              </button>
            </div>
            ...
            <div className="space-y-6">
              <div className="bg-card backdrop-blur-sm rounded-2xl p-8 border border-border shadow-xl sticky top-6">
                <div className="flex items-center mb-6">
                  ...
                  <h3 className="text-xl font-bold text-card-foreground">Order Summary</h3>
                </div>
                
                <div className="space-y-4">
                  <div className="flex justify-between text-muted-foreground">
                    <span>Subtotal ({cartItems.length} items)</span>
                    <span className="font-semibold text-card-foreground">₹{totals.subtotal.toLocaleString()}</span>
                  </div>
                  
                  <div className="flex justify-between text-muted-foreground">
                    <span>Shipping</span>
                    <span className={`font-semibold ${totals.shipping === 0 ? 'text-green-600 dark:text-green-400' : 'text-card-foreground'}`}>
                  ...
                  <div className="flex justify-between text-muted-foreground">
                    ...
                    <span className="font-semibold text-card-foreground">₹{totals.tax.toLocaleString()}</span>
                  </div>
                  ...
                  <div className="border-t border-border pt-4">
                    <div className="flex justify-between text-card-foreground font-bold text-xl">
                      <span>Total</span>
                      <span className="text-2xl bg-gradient-to-r from-primary via-primary/80 to-primary bg-clip-text text-transparent">₹{totals.total.toLocaleString()}</span>
                    </div>
                  </div>
                </div>
