      ...
      <div className="bg-slate-800 border-b border-slate-700 px-6 py-4">
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-slate-400">{wishlistItems.length} items saved for later</p>
          </div>
          ...
          <div className="flex items-center space-x-4">
            <div className="flex items-center space-x-3 bg-slate-700 rounded-lg px-3 py-2">
              ...
              <div className="text-right">
                <p className="text-sm font-medium text-white">{inStockCount} Available</p>
                <p className="text-xs text-slate-400">{wishlistItems.length} total items</p>
              </div>
            </div>
          </div>
        </div>
          ...
          <>
                ...
                >
                  <div className={`relative ${viewMode === 'list' ? 'w-32 h-32 flex-shrink-0' : 'w-full h-48'}`}>
                    ...
                    <button 
                      ...
                      className="absolute top-2 left-2 p-2 bg-slate-800/80 rounded-full hover:bg-red-500/80 transition-colors"
                    >
                  ...
                  <div className={`p-4 ${viewMode === 'list' ? 'flex-1' : ''}`}>
                    <div className={viewMode === 'list' ? 'flex items-center justify-between' : ''}>
                      <div className={viewMode === 'list' ? 'flex-1' : ''}>
                        ...
                        <p className="text-slate-400 text-sm mb-2">by {item.artisan}</p>
                        <p className="text-slate-400 text-xs mb-3">{item.location}</p>
                        ...
                        <div className="flex items-center justify-between mb-3">
                          <div>
                            ...
                            {item.originalPrice > item.price && (
                              <span className="text-slate-400 text-sm line-through ml-2">₹{item.originalPrice.toLocaleString()}</span>
                            )}
                          </div>
                          <span className="text-xs bg-slate-700 text-slate-300 px-2 py-1 rounded">{item.category}</span>
                        </div>

                        <p className="text-slate-400 text-xs mb-3">Added {new Date(item.addedDate).toLocaleDateString()}</p>
                      </div>
                      ...
                      <div className={`flex space-x-2 ${viewMode === 'list' ? 'ml-4 flex-col space-y-2 space-x-0' : ''}`}>
                        ...
                        <div className={`flex space-x-2 ${viewMode === 'list' ? '' : ''}`}>
                          ...
                          <button 
                            onClick={() => removeFromWishlist(item.id)}
                            className="p-2 border border-slate-600 rounded-lg hover:bg-red-500/20 hover:border-red-400 transition-colors"
                          >
            ...
            <div className="mt-8 bg-slate-800 rounded-lg p-6 border border-slate-700">
              <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                ...
                <div className="text-center">
                  <div className="text-2xl font-bold text-orange-500">{inStockCount}</div>
