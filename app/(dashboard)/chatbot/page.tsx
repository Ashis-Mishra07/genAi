    ...
    <div className="min-h-screen bg-background flex items-center justify-center">
      <div className="text-center space-y-4">
        <div className="flex items-center justify-center space-x-2">
          <div className="w-3 h-3 bg-blue-600 dark:bg-blue-400 rounded-full animate-bounce google-dot-1"></div>
          <div className="w-3 h-3 bg-red-600 dark:bg-red-400 rounded-full animate-bounce google-dot-2"></div>
          <div className="w-3 h-3 bg-yellow-600 dark:bg-yellow-400 rounded-full animate-bounce google-dot-3"></div>
          <div className="w-3 h-3 bg-green-600 dark:bg-green-400 rounded-full animate-bounce google-dot-4"></div>
        </div>
...
💡 **Pro Tip**: Once the poster is ready, I can also help you create a product listing directly for the artisan!`,
  ...
  const handleFileUpload = (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file && file.type.startsWith("image/")) {
      ...
      const assistantResponse: Message = {
        ...
        type: "assistant",
        content:
          "Great! I can see your product image. Now please tell me the product name so I can create a beautiful poster for you.",
        timestamp: new Date(),
      };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setTimeout(() => {
      const assistantResponse: Message = {
        ...
        type: "assistant",
        content:
          "I understand! To create a poster, please upload an image of your product first using the upload button, then tell me the product name.",
        timestamp: new Date(),
      };
  ...
  const handleThemeSelect = async (theme: PosterTheme) => {
    ...
    try {
      ...
      let productStory =
        "Premium quality construction with attention to detail. Designed for durability and functionality. Each piece showcases superior craftsmanship and materials.";
  ...
  const handlePosterReady = async (dataUrl: string) => {
    ...
    console.log("📋 Product Name:", currentPosterData.productName);
    console.log("🎭 Theme:", currentPosterData.theme.name);
    console.log("📏 Data URL Length:", dataUrl.length);
    console.log("===============================================");

    // Upload the generated poster to Cloudinary
    let cloudinaryUrl = dataUrl; // Fallback to dataUrl
    let uploadSuccess = false;

    try {
      console.log("☁️ Starting Cloudinary upload for generated poster...");

      const formData = new FormData();
      formData.append("dataUrl", dataUrl);
      formData.append("folder", "artisan-products/generated-posters");
      formData.append(
        "tags",
        `poster,generated,${currentPosterData.productName},${currentPosterData.theme.name}`
      );
      ...
      if (uploadResponse.ok) {
        ...
        if (uploadResult.success) {
          ...
          // 🎯 LOG THE CLOUDINARY URL HERE
          console.log("🌟 ===============================================");
          console.log("🎨 GENERATED POSTER UPLOADED TO CLOUDINARY!");
          console.log("📋 Product Name:", currentPosterData.productName);
          console.log("🎭 Theme:", currentPosterData.theme.name);
          console.log("🔗 Cloudinary URL:", cloudinaryUrl);
          console.log("📁 Folder: artisan-products/generated-posters");
          console.log(
            "🏷️ Tags: poster,generated," +
              currentPosterData.productName +
              "," +
              currentPosterData.theme.name
          );
          console.log("🆔 Public ID:", uploadResult.public_id);
          console.log("🎨 Format:", uploadResult.format);
          console.log("📊 File Type:", uploadResult.file_type);
          console.log("===============================================");
        } else {
          console.warn(
            "⚠️ Upload succeeded but response indicates failure:",
            uploadResult
          );
        }
      } else {
      ...
      console.warn(
        "⚠️ Failed to upload generated poster to Cloudinary, using dataUrl as fallback"
      );
    }

    // Create message with the professional poster (using Cloudinary URL if available)
    const posterMessage: Message = {
      id: (Date.now() + 2).toString(),
      type: "assistant",
      content: `🎨 **Your Premium ${
        currentPosterData.theme.name
      } Poster is Ready!** ✨
...
${
  uploadSuccess
    ? "☁️ **Cloud Storage Ready** - Securely stored in Cloudinary for fast global delivery"
    : ""
}
...
${
  uploadSuccess
    ...
    : "⚠️ **Note**: Using local storage (Cloudinary upload failed)"
}`,
      timestamp: new Date(),
      canvasPoster: {
        ...
        dataUrl: cloudinaryUrl, // Use Cloudinary URL if available, otherwise original dataUrl
    ...
    // Final summary log
    console.log("🏁 ===============================================");
    console.log("🏁 POSTER PROCESS COMPLETED");
    console.log("📋 Product:", currentPosterData.productName);
    console.log("🎭 Theme:", currentPosterData.theme.name);
    console.log("☁️ Cloudinary Upload:", uploadSuccess ? "SUCCESS" : "FAILED");
    console.log(
      "🔗 Final URL Type:",
      cloudinaryUrl.includes("cloudinary.com") ? "Cloudinary URL" : "Data URL"
    );
  ...
  const drawAdvancedBackground = (
    ...
    switch (theme.layout) {
      case "luxe":
        ...
        const luxeGradient = ctx.createRadialGradient(
          width / 3,
          height / 3,
          0,
          width / 2,
  ...
  const drawProductShowcase = (
    ...
    const frameGradient = ctx.createLinearGradient(
      imgX - 20,
      imgY - 20,
      imgX + displayWidth + 20,
      imgY + displayHeight + 20
    );
    ...
    try {
      ...
      if (response.ok) {
        ...
        if (data.conversations && data.conversations.length > 0) {
          const artisanConversation = data.conversations.find(
            (conv: any) => conv.artisan_id === artisanId
          );
  ...
  const createProductForArtisan = async (posterData: any, price: string) => {
    if (!artisanInfo) {
      ...
      if (!artisanInfo) {
        alert(
          "Unable to identify the artisan. Please try going back to the chat and creating the poster again."
        );
        return;
      }
    }
    ...
    try {
      ...
      const productData = {
        ...
        description:
          posterData.productStory ||
          `Beautiful ${
            posterData.productName || "handcrafted item"
          } created by ${artisanInfo.name}`,
      ...
      if (createResponse.ok) {
        alert(
          `🎉 Product "${productData.name}" created successfully for ${artisanInfo.name}!\n\nPrice: $${price}\nThe product is now live with your beautiful poster!`
        );
...
Your product is now live and ready for customers! The beautiful poster we created together will help attract buyers. 🌟`;
          ...
          console.log(
            "Note: Could not send notification to artisan, but product was created successfully"
          );
        }
      } else {
  ...
  const copyToClipboard = async (dataUrl: string) => {
    try {
      ...
      alert(t("Poster copied to clipboard!"));
    } catch (error) {
      console.error("Error copying to clipboard:", error);
      alert(t("Failed to copy poster"));
    }
  };
  ...
  return (
    <div className="flex flex-col h-full bg-background">
      ...
      {/* Header */}
      <div className="flex-shrink-0 p-6 bg-card/95 backdrop-blur-sm border-b border-border shadow-sm">
          ...
          <div>
            <h1 className="text-2xl font-bold text-foreground">
              {searchParams.get("mode") === "poster-creation"
                ? `🎨 ${t("Admin Poster Creator")}`
                : t("AI Poster Creator")}
            </h1>
            <p className="text-sm text-muted-foreground">
              {searchParams.get("mode") === "poster-creation"
                ? artisanInfo
                  ? `${t("Creating marketing poster for")} ${artisanInfo.name}`
                  : t("Creating marketing poster for artisan")
                : t("Create stunning storytelling posters")}
            </p>
            {/* Artisan Info Badge */}
            {searchParams.get("mode") === "poster-creation" && artisanInfo && (
              <div className="flex items-center gap-2 mt-2 px-3 py-1 bg-green-500/10 border border-green-500/30 rounded-full w-fit">
                <User className="w-3 h-3 text-green-600 dark:text-green-400" />
                <span className="text-xs text-green-600 dark:text-green-400 font-medium">
                  {t("Helping:")} {artisanInfo.name}
                </span>
              </div>
            )}
          </div>
      ...
      <div className="flex-1 overflow-y-auto p-6 space-y-4">
          ...
          <div
            ...
            <div
              className={`max-w-2xl p-4 rounded-2xl ${
                message.type === "user"
                  ? "bg-primary text-primary-foreground font-medium ml-12 shadow-lg"
                  : "bg-card backdrop-blur-sm text-foreground mr-12 border border-border shadow-lg"
              }`}>
              <div className="whitespace-pre-wrap">{message.content}</div>

              {/* Canvas Poster Display */}
              {message.canvasPoster && (
                <div className="mt-4 p-4 bg-accent/50 backdrop-blur-sm rounded-xl border border-border">
                  ...
                  <div className="bg-background/80 p-4 rounded-lg shadow-lg border border-border">
                    <img
                      src={message.canvasPoster.dataUrl}
                      alt={`Poster for ${message.canvasPoster.productName}`}
                      className="w-full max-w-sm mx-auto rounded-lg shadow-2xl border border-border"
                    />
                  </div>
                  ...
                  <div className="flex gap-2 mt-4">
                    <button
                      ...
                      className="flex items-center gap-2 px-4 py-2 bg-primary hover:bg-primary/90 text-primary-foreground font-medium rounded-lg transition-all duration-200 text-sm shadow-lg">
                    ...
                    <button
                      ...
                      className="flex items-center gap-2 px-4 py-2 bg-secondary hover:bg-secondary/90 text-secondary-foreground rounded-lg transition-all duration-200 text-sm border border-border">
                    ...
                    {searchParams.get("mode") === "poster-creation" && (
                      <button
                        onClick={() => {
                          if (!artisanInfo) {
                            alert(
                              "Artisan information is still loading. Please wait a moment and try again."
                            );
                            return;
                          }
                          const price = prompt(
                            t("Enter the product price (in $):") || "Enter the product price (in $):",
                            "25"
                          );
                          if (price && message.canvasPoster) {
                            createProductForArtisan(
                              message.canvasPoster,
                              price
                            );
                          }
                        }}
                        className="flex items-center gap-2 px-4 py-2 bg-green-600 hover:bg-green-700 dark:bg-green-500 dark:hover:bg-green-600 text-white font-medium rounded-lg transition-all duration-200 text-sm shadow-lg">
                        <Package className="w-4 h-4" />
                        {artisanInfo
                          ? `${t("Create Product for")} ${artisanInfo.name}`
                          : t("Create Product")}
                      </button>
                    )}
                  </div>
        ...
        {isLoading && (
          <div className="flex justify-start">
            <div className="bg-card backdrop-blur-sm text-foreground p-4 rounded-2xl mr-12 border border-border shadow-lg">
              <div className="flex items-center gap-2">
                ...
                <span className="text-muted-foreground text-sm ml-2">
                  {t("Creating your stunning poster...")}
                </span>
              </div>
            </div>
          </div>
      ...
      {showThemeSelector && (
        <div className="fixed inset-0 bg-black/60 dark:bg-black/80 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <div className="bg-card border border-border rounded-2xl p-6 max-w-4xl w-full max-h-[80vh] overflow-y-auto shadow-2xl">
            <div className="flex items-center justify-between mb-6">
              ...
              <button
                ...
                className="text-muted-foreground hover:text-foreground text-xl font-bold w-8 h-8 flex items-center justify-center rounded-lg hover:bg-accent transition-all duration-200">
                ✕
              </button>
            </div>
      ...
      {/* Input Area */}
      <div className="flex-shrink-0 p-6 bg-card/95 backdrop-blur-sm border-t border-border shadow-sm">
        <form onSubmit={handleSubmit} className="flex gap-3">
          ...
          <button
            ...
            className="flex items-center justify-center w-12 h-12 bg-primary hover:bg-primary/90 text-primary-foreground font-bold rounded-xl transition-all duration-200 shadow-lg hover:scale-105">
          ...
          <input
            ...
            placeholder={
              selectedFile && !productName
                ? t("Enter product name...")
                : t("Type your message...")
            }
            className="flex-1 px-4 py-3 bg-background border border-border rounded-xl focus:outline-none focus:ring-2 focus:ring-primary focus:border-primary text-foreground placeholder-muted-foreground backdrop-blur-sm"
            disabled={isLoading}
          />
          ...
          <button
            ...
            className="flex items-center justify-center w-12 h-12 bg-primary hover:bg-primary/90 text-primary-foreground font-bold rounded-xl transition-all duration-200 shadow-lg hover:scale-105 disabled:opacity-50 disabled:cursor-not-allowed disabled:hover:scale-100">
            <Send className="w-5 h-5" />
          </button>
        </form>

        {selectedFile && (
          <div className="mt-3 flex items-center gap-2 text-sm text-foreground bg-accent/50 p-3 rounded-lg border border-border">
